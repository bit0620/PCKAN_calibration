...
#     model = torch.load(
#         f'../neural_network/train_res/{save_dir}/PCKAN0.00017504163937849033.pt',
#         map_location=device
#     )
#     model.eval()
...
#         calibration_params_df.to_csv(f"../data/calibration_params/{save_dir}/{net_name}_120.csv", index=False)
#     else:
#         calibration_params_df = pd.DataFrame(calibration_params)
#         calibration_params_df.to_csv(f"../data/calibration_params/{save_dir}/{net_name}.csv", index=False)
...
#     train_data = pd.read_csv('../data/train_data_p.csv').to_numpy()
#     option_params_train = train_data[:, :-1]
#     real_prices_train = train_data[:, -1].reshape(-1, 1)

#     test_data = pd.read_csv('../data/test_data_p.csv').to_numpy()
#     option_params_test = test_data[:, :-1]
#     real_prices_test = test_data[:, -1].reshape(-1, 1)
