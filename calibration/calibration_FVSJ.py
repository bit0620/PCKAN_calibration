...
train_data = pd.read_csv('../data/train_data.csv')
train_data = np.array(train_data)
train_data_tensor = torch.tensor(train_data[:5, :], dtype=torch.float32).to(device)
...
calibration_params_df = pd.DataFrame(calibration_params)
calibration_params_df.to_csv("../data/calibration_params/FVSJ/FVSJ_params_5.csv", index=False)
